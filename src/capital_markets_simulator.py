...
def show_capital_markets_simulator():
    ...
    with st.form("financing_form"):
        ...
        with col2:
            interest_rate_pct = st.slider("Interest Rate %", 3.0, 12.0, cap_details.interest_rate_pct, 0.125, format="%.3f%%")
        with col3:
            term_years = st.selectbox("Loan Term (Years)", [30, 25, 20, 15], index=0)

        closing_costs_pct = st.slider(
            "Estimated Closing Costs % (of Purchase Price)", 
            0, 7, cap_details.closing_costs_pct, 1,
        ...
        submitted = st.form_submit_button("Save Financing Details")
        if submitted:
            st.session_state.deal_profile.capital_markets_details = CapitalMarketsDetails(
                loan_type=loan_type, ltv_pct=ltv_pct, interest_rate_pct=interest_rate_pct,
                term_years=term_years, closing_costs_pct=closing_costs_pct, loan_amount=loan_amount,
                down_payment=down_payment, closing_costs=closing_costs, monthly_payment=monthly_payment,
                annual_debt_service=annual_debt_service,
            )
            st.success("Financing details have been saved to the current deal profile!")